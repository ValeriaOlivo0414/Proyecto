...
from app.views.barcode_view import BarcodeView

class MainView(tk.Tk):
    def __init__(self, estudiante_controller, estudiante_action_controller):
        super().__init__()
        self.estudiante_controller = estudiante_controller
        self.estudiante_action_controller = estudiante_action_controller
    ...
    def open_barcode_view(self):
        ...
        barcode_app = BarcodeView(self.estudiante_action_controller, master=barcode_window)
    ...
    def search_estudiante(self):
        ...
        estudiante = self.estudiante_controller.buscar_estudiante(documento)
        if estudiante:
    ...
    def insert_estudiante(self):
        ...
        estudiante = self.estudiante_controller.crear_estudiante(*values)
        if estudiante:
    ...
    def update_estudiante(self):
        values = [self.entries[field].get() for field in self.fields]
        if self.estudiante_controller.actualizar_estudiante(*values):
    ...
    def delete_estudiante(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este estudiante?"):
            if self.estudiante_controller.eliminar_estudiante(documento):
                messagebox.showinfo("Éxito", "Estudiante eliminado correctamente")
                self.clear_fields()
                self.show_all_estudiante()

    def show_all_estudiante(self):
        estudiantes = self.estudiante_controller.ver_estudiantes()
